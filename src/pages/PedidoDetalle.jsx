...
export default function PedidoDetalle() {
  ...
  const [panelItem, setPanelItem] = useState(null)     // pc seleccionado
  const [panelPago, setPanelPago] = useState(null)     // pc seleccionado
  ...
  const quitarCliente = async (clienteId) => {
    ...
    try {
      await removeClienteFromPedido(id, clienteId)
      cargar()
    } catch {
      toast.error('Error al quitar cliente')
    }
  }
  ...
  const toggleLlegado = async (pc, item) => {
    try {
      await updateItem(pc.id, item.id, { llegado: !item.llegado })
      cargar()
    } catch {
  ...
  const subirImagenItem = async (pc, itemId, file) => {
    try {
      await uploadItemImagen(pc.id, itemId, file)
  ...
  const subirComprobante = async (pc, pagoId, file) => {
    try {
      await uploadComprobante(pc.id, pagoId, file)
  ...
  return (
    <div>
      ...
      {pedido.clientes.length === 0 ? (
        ...
        <div className="space-y-5">
          {pedido.clientes.map((pc) => (
            <div key={pc.id} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
              {/* Header cliente */}
              <div className="flex items-center justify-between px-5 py-3 bg-gray-50 border-b border-gray-200">
                <div>
                  <span className="font-semibold text-gray-800">{pc.cliente_nombre}</span>
                ...
                <div className="flex items-center gap-4">
                  <button
                    onClick={() => {
                      sessionStorage.setItem('pedido_id_para_factura', id)
                      navigate(`/factura/${pc.id}`)
                    }}
                    className="text-sm text-blue-600 hover:underline"
                  >
                    Ver factura
                  </button>
                  <button onClick={() => quitarCliente(pc.cliente_id)} className="text-sm text-red-400 hover:underline">
                    Quitar
                  </button>
                </div>
              </div>
              ...
              <div className="p-5 space-y-4">
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-sm font-medium text-gray-600">Articulos</span>
                    <button
                      ...
                      {pc.items.map((item) => (
                        <div key={item.id} className="flex items-center gap-3 py-1">
                          ...
                          <div className="flex-1 min-w-0">
                            {item.articulo && (
                              <p className="text-sm text-gray-700 truncate">{item.articulo}</p>
                            )}
                            {item.link && (
                              <a
                                href={item.link}
                            ...
                            {!item.articulo && !item.link && (
                              <span className="text-xs text-gray-400">Item #{item.numero}</span>
                            )}
                          </div>
                          <div className="flex items-center gap-2 flex-shrink-0">
                            <span className="text-sm font-medium">${Number(item.precio).toFixed(2)}</span>
                            <button
                            ...
                            <button
                              onClick={() => eliminarItem(pc, item.id)}
                              className="text-gray-300 hover:text-red-400 text-sm w-5 h-5 flex items-center justify-center"
                            >
                              &times;
                            </button>
                          </div>
                ...
                <div className="border-t border-gray-100 pt-3 space-y-1 text-sm">
                  ...
                  <div className="flex justify-between text-gray-500">
                    <span>Pagado</span>
                    <span className="text-green-600">-${Number(pc.total_pagado).toFixed(2)}</span>
                  </div>
                  <div className={`flex justify-between font-bold text-base pt-1 border-t border-gray-100 ${Number(pc.saldo) > 0 ? 'text-red-600' : 'text-green-600'}`}>
                        ...
                        <div key={pago.id} className="flex items-center gap-3 py-1">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-gray-400">
                              {new Date(pago.fecha).toLocaleDateString('es', {
                          ...
                          <button
                            ...
                            className="text-gray-300 hover:text-red-400 text-sm w-5 h-5 flex items-center justify-center flex-shrink-0"
                          >
                            &times;
                          </button>
                        </div>
      ...
      <SidePanel
        ...
        title={panelItem ? `Nuevo articulo — ${panelItem.cliente_nombre}` : ''}
      >
        {panelItem && (
          <form onSubmit={(e) => agregarItem(e, panelItem)} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Link de Temu</label>
              <input
                type="text"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setFormItem({ ...formItem, articulo: e.target.value })}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
            <div>
        ...
        {panelPago && (
          <form onSubmit={(e) => registrarPago(e, panelPago)} className="space-y-4">
            ...
            <div>
              ...
              <select
                value={formPago.tipo}
                onChange={(e) => setFormPago({ ...formPago, tipo: e.target.value })}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
