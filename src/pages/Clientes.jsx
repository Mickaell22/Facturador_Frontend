...
export default function Clientes() {
  ...
  const abrirEditar = (cliente) => {
    setEditando(cliente)
    setForm({ nombre: cliente.nombre, comision_por_item: String(cliente.comision_por_item) })
    setPanelAbierto(true)
  }
    ...
    try {
      if (editando) {
        await updateCliente(editando.id, {
          nombre: form.nombre,
          comision_por_item: parseFloat(form.comision_por_item),
        })
  ...
  const handleEliminar = async (id) => {
    if (!confirm('Eliminar cliente? Se perderan todos sus pedidos asociados.')) return
    try {
  ...
  const handleAgregarAlias = async (clienteId) => {
    ...
    if (!alias) return
    try {
      await addAlias(clienteId, alias)
      setNuevoAlias({ ...nuevoAlias, [clienteId]: '' })
      cargar()
    } catch {
      toast.error('Ese alias ya existe')
    }
  }

  const handleEliminarAlias = async (clienteId, aliasId) => {
    try {
      await deleteAlias(clienteId, aliasId)
      cargar()
    } catch {
            ...
            <div key={c.id} className="bg-white border border-gray-200 rounded-xl p-5">
              <div className="flex items-start justify-between mb-3">
                <div>
                  ...
                  <p className="text-sm text-gray-400 mt-0.5">
                    Comision: ${Number(c.comision_por_item).toFixed(2)}/item
                  </p>
                </div>
                <div className="flex gap-3">
                  <button onClick={() => abrirEditar(c)} className="text-sm text-blue-500 hover:underline">
                    Editar
                  </button>
                  <button onClick={() => handleEliminar(c.id)} className="text-sm text-red-400 hover:underline">
                    Eliminar
                  </button>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  {c.aliases.length === 0 && (
                    <span className="text-xs text-gray-300">Sin aliases</span>
                  )}
                  {c.aliases.map((a) => (
                    <span
                      key={a.id}
                    ...
                    >
                      {a.alias}
                      <button
                        onClick={() => handleEliminarAlias(c.id, a.id)}
