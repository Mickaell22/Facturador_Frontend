...
export default function Factura() {
  ...
  useEffect(() => {
    const cargar = async () => {
      try {
        // pcId pertenece a un pedido, buscamos via la API de pedidos
        // Necesitamos el pedido_id — lo obtenemos desde la URL o state
        // Por ahora usamos el pedido_id guardado en sessionStorage
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-6 px-4">
      ...
      <div
        ref={facturaRef}
        className="bg-white max-w-lg mx-auto rounded-xl shadow-sm p-6 space-y-5"
      >
        {/* Header */}
            ...
            <div className="space-y-2">
              {pc.items.map((item) => (
                <div key={item.id} className="flex items-center gap-3">
                  {item.imagen_url && (
                    <img src={item.imagen_url} alt="" className="w-10 h-10 object-cover rounded-lg border border-gray-100" />
                  )}
                  ...
                  <div className="text-right flex-shrink-0">
                    <p className={`text-sm font-medium ${!item.llegado ? 'text-gray-300' : ''}`}>
        ...
        <div className={`rounded-xl p-4 ${Number(pc.saldo) <= 0 ? 'bg-green-50' : 'bg-red-50'}`}>
          ...
          {Number(pc.saldo) <= 0 && (
            <p className="text-green-600 text-xs mt-1">Pagado en su totalidad</p>
          )}
        </div>
      </div>
    </div>
