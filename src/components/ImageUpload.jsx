...
export default function ImageUpload({ imageUrl, onUpload, label = 'foto' }) {
  ...
  // Listener global de paste — solo activo cuando este slot está enfocado
  useEffect(() => {
    if (!activo) return
    const handler = (e) => {
  ...
  return (
    ...
    >
      <span className={`text-xs leading-tight text-center pointer-events-none ${activo ? 'text-blue-500 font-medium' : 'text-gray-300'}`}>
        {activo ? 'Ctrl+V' : label}
      </span>
    </div>
  )
